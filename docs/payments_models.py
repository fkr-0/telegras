"""Telegram Bot API - Payments Pydantic Models.
...
class getStarTransactions(BaseModel):
    ...
    offset: None | int = Field(description="Number of transactions to skip in the response")
    limit: None | int = Field(description="The maximum number of transactions to be retrieved. Values between 1-100 are accepted. Defaults to 100.")
...
class AffiliateInfo(BaseModel):
    ...
    affiliate_user: None | User = Field(description="Optional . The bot or the user that received an affiliate commission if it was received by a bot or a user")
    affiliate_chat: None | Chat = Field(description="Optional . The chat that received an affiliate commission if it was received by a chat")
    ...
    nanostar_amount: None | int = Field(description="Optional . The number of 1/1000000000 shares of Telegram Stars received by the affiliate; from -999999999 to 999999999; can be negative for refunds")
...
class TransactionPartnerChat(BaseModel):
    ...
    gift: None | Gift = Field(description="Optional . The gift sent to the chat by the bot")
...
class TransactionPartner(BaseModel):
    ...
    affiliate: None | AffiliateInfo = Field(description="Optional . Information about the affiliate that received a commission via this transaction. Can be available only for “invoice_payment” and “paid_media_payment” transactions.")
    invoice_payload: None | str = Field(description="Optional . Bot-specified invoice payload. Can be available only for “invoice_payment” transactions.")
    subscription_period: None | int = Field(description="Optional . The duration of the paid subscription. Can be available only for “invoice_payment” transactions.")
    paid_media: None | list[PaidMedia] = Field(description="Optional . Information about the paid media bought by the user; for “paid_media_payment” transactions only")
    paid_media_payload: None | str = Field(description="Optional . Bot-specified paid media payload. Can be available only for “paid_media_payment” transactions.")
    gift: None | Gift = Field(description="Optional . The gift sent to the user by the bot; for “gift_purchase” transactions only")
    premium_subscription_duration: None | int = Field(description="Optional . Number of months the gifted Telegram Premium subscription will be active for; for “premium_purchase” transactions only")
...
class TransactionPartnerAffiliateProgram(BaseModel):
    ...
    sponsor_user: None | User = Field(description="Optional . Information about the bot that sponsored the affiliate program")
...
class RefundedPayment(BaseModel):
    ...
    telegram_payment_charge_id: str = Field(description="Telegram payment identifier")
    provider_payment_charge_id: None | str = Field(description="Optional . Provider payment identifier")
...
class StarTransaction(BaseModel):
    ...
    nanostar_amount: None | int = Field(description="Optional . The number of 1/1000000000 shares of Telegram Stars transferred by the transaction; from 0 to 999999999")
    date: int = Field(description="Date the transaction was created in Unix time")
    source: None | TransactionPartner = Field(description="Optional . Source of an incoming transaction (e.g., a user purchasing goods or services, Fragment refunding a failed withdrawal). Only for incoming transactions")
    receiver: None | TransactionPartner = Field(description="Optional . Receiver of an outgoing transaction (e.g., a user for a purchase refund, Fragment for a withdrawal). Only for outgoing transactions")
...
class TransactionPartnerFragment(BaseModel):
    ...
    withdrawal_state: None | RevenueWithdrawalState = Field(description="Optional . State of the transaction if the transaction is outgoing")
...
class createInvoiceLink(BaseModel):
    ...
    business_connection_id: None | str = Field(description="Unique identifier of the business connection on behalf of which the link will be created. For payments in Telegram Stars only.")
    ...
    provider_token: None | str = Field(description="Payment provider token, obtained via @BotFather . Pass an empty string for payments in Telegram Stars .")
    ...
    subscription_period: None | int = Field(description="The number of seconds the subscription will be active for before the next payment. The currency must be set to “XTR” (Telegram Stars) if the parameter is used. Currently, it must always be 2592000 (30 days) if specified. Any number of subscriptions can be active for a given bot at the same time, including multiple concurrent subscriptions from the same user. Subscription price must no exceed 10000 Telegram Stars.")
    max_tip_amount: None | int = Field(description="The maximum accepted amount for tips in the smallest units of the currency (integer, not float/double). For example, for a maximum tip of US$ 1.45 pass max_tip_amount = 145 . See the exp parameter in currencies.json , it shows the number of digits past the decimal point for each currency (2 for the majority of currencies). Defaults to 0. Not supported for payments in Telegram Stars .")
    suggested_tip_amounts: None | list[int] = Field(description="A JSON-serialized array of suggested amounts of tips in the smallest units of the currency (integer, not float/double). At most 4 suggested tip amounts can be specified. The suggested tip amounts must be positive, passed in a strictly increased order and must not exceed max_tip_amount .")
    provider_data: None | str = Field(description="JSON-serialized data about the invoice, which will be shared with the payment provider. A detailed description of required fields should be provided by the payment provider.")
    photo_url: None | str = Field(description="URL of the product photo for the invoice. Can be a photo of the goods or a marketing image for a service.")
    photo_size: None | int = Field(description="Photo size in bytes")
    photo_width: None | int = Field(description="Photo width")
    photo_height: None | int = Field(description="Photo height")
    need_name: None | bool = Field(description="Pass True if you require the user's full name to complete the order. Ignored for payments in Telegram Stars .")
    need_phone_number: None | bool = Field(description="Pass True if you require the user's phone number to complete the order. Ignored for payments in Telegram Stars .")
    need_email: None | bool = Field(description="Pass True if you require the user's email address to complete the order. Ignored for payments in Telegram Stars .")
    need_shipping_address: None | bool = Field(description="Pass True if you require the user's shipping address to complete the order. Ignored for payments in Telegram Stars .")
    send_phone_number_to_provider: None | bool = Field(description="Pass True if the user's phone number should be sent to the provider. Ignored for payments in Telegram Stars .")
    send_email_to_provider: None | bool = Field(description="Pass True if the user's email address should be sent to the provider. Ignored for payments in Telegram Stars .")
    is_flexible: None | bool = Field(description="Pass True if the final price depends on the shipping method. Ignored for payments in Telegram Stars .")
...
class OrderInfo(BaseModel):
    ...
    name: None | str = Field(description="Optional . User name")
    phone_number: None | str = Field(description="Optional . User's phone number")
    email: None | str = Field(description="Optional . User email")
    shipping_address: None | ShippingAddress = Field(description="Optional . User shipping address")
...
class answerShippingQuery(BaseModel):
    ...
    shipping_options: None | list[ShippingOption] = Field(description="Required if ok is True . A JSON-serialized array of available shipping options.")
    error_message: None | str = Field(description="Required if ok is False . Error message in human readable form that explains why it is impossible to complete the order (e.g. “Sorry, delivery to your desired address is unavailable”). Telegram will display this message to the user.")
...
class sendInvoice(BaseModel):
    ...
    max_tip_amount: None | int = Field(description="The maximum accepted amount for tips in the smallest units of the currency (integer, not float/double). For example, for a maximum tip of US$ 1.45 pass max_tip_amount = 145 . See the exp parameter in currencies.json , it shows the number of digits past the decimal point for each currency (2 for the majority of currencies). Defaults to 0. Not supported for payments in Telegram Stars .")
    suggested_tip_amounts: None | list[int] = Field(description="A JSON-serialized array of suggested amounts of tips in the smallest units of the currency (integer, not float/double). At most 4 suggested tip amounts can be specified. The suggested tip amounts must be positive, passed in a strictly increased order and must not exceed max_tip_amount .")
    start_parameter: None | str = Field(description="Unique deep-linking parameter. If left empty, forwarded copies of the sent message will have a Pay button, allowing multiple users to pay directly from the forwarded message, using the same invoice. If non-empty, forwarded copies of the sent message will have a URL button with a deep link to the bot (instead of a Pay button), with the value used as the start parameter")
    provider_data: None | str = Field(description="JSON-serialized data about the invoice, which will be shared with the payment provider. A detailed description of required fields should be provided by the payment provider.")
    photo_url: None | str = Field(description="URL of the product photo for the invoice. Can be a photo of the goods or a marketing image for a service. People like it better when they see what they are paying for.")
    photo_size: None | int = Field(description="Photo size in bytes")
    photo_width: None | int = Field(description="Photo width")
    photo_height: None | int = Field(description="Photo height")
    need_name: None | bool = Field(description="Pass True if you require the user's full name to complete the order. Ignored for payments in Telegram Stars .")
    ...
    reply_markup: None | InlineKeyboardMarkup = Field(description="A JSON-serialized object for an inline keyboard . If empty, one 'Pay total price ' button will be shown. If not empty, the first button must be a Pay button.")
...
class PreCheckoutQuery(BaseModel):
    ...
    total_amount: int = Field(description="Total price in the smallest units of the currency (integer, not float/double). For example, for a price of US$ 1.45 pass amount = 145 . See the exp parameter in currencies.json , it shows the number of digits past the decimal point for each currency (2 for the majority of currencies).")
    invoice_payload: str = Field(description="Bot-specified invoice payload")
    shipping_option_id: None | str = Field(description="Optional . Identifier of the shipping option chosen by the user")
    order_info: None | OrderInfo = Field(description="Optional . Order information provided by the user")
...
class SuccessfulPayment(BaseModel):
    ...
    total_amount: int = Field(description="Total price in the smallest units of the currency (integer, not float/double). For example, for a price of US$ 1.45 pass amount = 145 . See the exp parameter in currencies.json , it shows the number of digits past the decimal point for each currency (2 for the majority of currencies).")
    invoice_payload: str = Field(description="Bot-specified invoice payload")
    subscription_expiration_date: None | int = Field(description="Optional . Expiration date of the subscription, in Unix time; for recurring payments only")
    is_recurring: None | bool = Field(description="Optional . True , if the payment is a recurring payment for a subscription")
    is_first_recurring: None | bool = Field(description="Optional . True , if the payment is the first payment for a subscription")
    shipping_option_id: None | str = Field(description="Optional . Identifier of the shipping option chosen by the user")
    order_info: None | OrderInfo = Field(description="Optional . Order information provided by the user")
    telegram_payment_charge_id: str = Field(description="Telegram payment identifier")
